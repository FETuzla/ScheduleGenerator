...
export class CustomSchedule {
  ...
  public addLecture(lecture: Lecture) {
    const isDuplicate = this.myLectures.some(
      (l) => l.name === lecture.name && l.displayName === lecture.displayName && l.day === lecture.day && l.startTime === lecture.startTime,
    );
    if (isDuplicate) {
  ...
  public removeLecture(lecture: Lecture) {
    this.myLectures = this.myLectures.filter(
      (l) =>
        !(l.name === lecture.name && l.displayName === lecture.displayName && l.day === lecture.day && l.startTime === lecture.startTime),
    );
  }
