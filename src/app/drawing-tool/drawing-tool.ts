...
export class DrawingTool implements AfterViewInit, OnChanges {
  ...
  private updateProcessedLectures() {
    ...
    this.lectures.forEach((lec) => {
      const isWeekend = ['saturday', 'subota', 'sunday', 'nedjelja'].includes(
        lec.day.toLowerCase(),
      );
      ...
      while (!fontValid && fontSize > 4) {
        ...
        if (fontValid) {
          ...
          teacherList.forEach((teacher) => {
            this.getWrappedLines(teacher, maxWidth).forEach((line) =>
              wrappedTeacherLines.push(line),
            );
          });
  ...
  generateIcs() {
    ...
    let ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//ScheduleGenerator//EN'];
